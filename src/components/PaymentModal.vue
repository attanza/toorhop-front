            ...
            <v-stepper-content step="2">
              <div v-if="e1 === 2">
                ...
                <v-layout
                  row
                  wrap
                  v-if="currentPayment && currentPayment.slug === 'credit-card' && !midtransUrl"
                  class="mb-4"
                >
                ...
                <v-layout row wrap v-if="midtransUrl">
                  <v-flex xs12>
                    <iframe
                      :src="midtransUrl"
                      frameborder="0"
    ...
    async getToken () {
      ...
      const callback = async response => {
        if (response.redirect_url) {
          ...
          // Submit form
          this.orderDetail.token_id = response.token_id
          await this.process()
        } else {
          this.midtransUrl = null
